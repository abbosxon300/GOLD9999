...
# ---- Sozlamalar: Agent (Variant A: sotuvchi xodim) ----
...
@app.route("/settings/agents/add", methods=["POST"])
@login_required
@admin_required
def settings_agents_add():
    ...
    try:
        exec_sql(
            ...
            (username, generate_password_hash(password), full_name, "agent")
        )
        flash("Agent qo‘shildi ✅", "success")
    except sqlite3.IntegrityError:
        flash("Bu username band", "danger")
    return redirect(url_for("settings_agents"))

@app.route("/settings/agents/toggle/<int:user_id>", methods=["POST"])
@login_required
@admin_required
...
# Auto DB init on import (init sahifa yo'q)
with app.app_context():
    init_db()



@app.post("/sales/qty/<int:product_id>/<action>")
@login_required
